...
- (void)viewDidLoad
{
    ...
    [SJOPaperboyViewController hasBackgroundModeEnabled];
    ...
    self.locationManager = [SJOPaperboyLocationManager sharedLocationManager];
    [[SJOPaperboyLocationManager sharedInstance] setLocationChangedBlock:^{
        ...
        self.title = NSLocalizedStringFromTable(@"view_controller_title", @"Netservis", nil);
    ...
    if ([SJOPaperboyViewController hasBackgroundModeEnabled]) {
        headerText = [NSString stringWithFormat:NSLocalizedStringFromTable(@"header_text_plus_fetch", @"Netservis", nil), appName];
    } else {
        headerText = [NSString stringWithFormat:NSLocalizedStringFromTable(@"header_text", @"Netservis", nil), appName];
    }
    
    NSString* footerText = [NSString stringWithFormat:NSLocalizedStringFromTable(@"footer_text", @"Netservis", nil), appName, deviceName];
    ...
    switch (indexPath.section) {
            ...
            switch (indexPath.row) {
                case 0:
                    if ([SJOPaperboyViewController hasBackgroundModeEnabled]) {
                        ...
                        cell.textLabel.text = NSLocalizedStringFromTable(@"background_updates", @"Netservis", nil);
                        [toggleSwitch addTarget:self action:@selector(toggleBackgroundUpdates:) forControlEvents:UIControlEventValueChanged];
                        break;
                    }
                default:
                    cell.userInteractionEnabled = [CLLocationManager isMonitoringAvailableForClass:[CLBeaconRegion class]];
                    toggleSwitch.userInteractionEnabled = [CLLocationManager isMonitoringAvailableForClass:[CLBeaconRegion class]];
                    toggleSwitch.on = [SJOPaperboyViewController isLocationUpdatingEnabled];
                    cell.textLabel.text = NSLocalizedStringFromTable(@"location_updates", @"Netservis", nil);
        ...
        case 1:
        {
            ...
            switch (indexPath.row) {
                case 0:
                    if (self.isLocating) {
                        cell.textLabel.text = NSLocalizedStringFromTable(@"getting_location", @"Netservis", nil);
                        UIActivityIndicatorView* loadingIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
                        [loadingIndicator startAnimating];
                        cell.accessoryView = loadingIndicator;
                    } else {
                        cell.textLabel.text = NSLocalizedStringFromTable(@"add_location", @"Netservis", nil);
                        cell.accessoryView = nil;
                    }
                    break;
                case 1:
                    cell.textLabel.text = NSLocalizedStringFromTable(@"clear_locations", @"Netservis", nil);
                    cell.accessoryView = nil;
                    break;
            }
        ...
        case 2: {
            NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
            NSDictionary *savedLocations = [userDefaults objectForKey:kLocations];
            NSString* location = [savedLocations allKeys][indexPath.row];
    ...
    switch (indexPath.section) {
        ...
        case 1:
        {
            switch (indexPath.row) {
                ...
                case 1: {
                    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
                    [userDefaults setObject:@{} forKey:kLocations];
                    [userDefaults synchronize];
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    [geoCoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray *placemarks, NSError *error) {
        for (CLPlacemark * placemark in placemarks) {
            ...
            savedLocations[addressString] = [NSKeyedArchiver archivedDataWithRootObject:currentLocation];
...
-(void) updateGeofencedLocations
{
    ...
    for (int i = 0; i < [regionArray count]; i++) {
        [[SJOPaperboyLocationManager sharedLocationManager] stopMonitoringForRegion:regionArray[i]];
    }
    ...
    if ([SJOPaperboyViewController isLocationUpdatingEnabled]) {
        ...
        for(CLLocation *location in locations) {
            ...
            CLCircularRegion *geofence = [[CLCircularRegion alloc] initWithCenter:location.coordinate
                                                                         radius:10
